...
## Minefield repair — 2026-09-14

The old board displayed counts before reveal, discarded its shuffled layout,
and never detected wins. Safe clicks only painted over an existing circle;
the previous browser smoke test checked for errors, not game progress.

The game now places exactly 26 mines after the first click, excluding that
cell and its neighbors. Empty regions expand, numbered cells reveal on click,
flags protect unopened cells, and clearing all 170 safe cells wins. A mine hit
ends the round; New game clears all state. Canvas coordinates account for CSS
scaling and Retina density. Gameplay no longer depends on external libraries.

Reproduce validation:

```sh
node tests/minefield.test.cjs
python3 tests/check_minefield.py
```

The model test uses 100 seeded synthetic boards and an independent neighbor
count oracle. The browser test uses seed 42 at 1440px/DPR 2 and 390px/DPR 3,
