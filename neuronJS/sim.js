...
(function () {
  ...
  const DELAY_BUFFER = 10;    // ring buffer length for propagation delay
  const MAX_DELAY_STEPS = 8;
  ...
  function buildDistanceConnectivity(n, positions, avgDegree) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      let count = 0;
      for (let c = 0; c < candidates.length && count < K_MAX; c++) {
  ...
  function updateRaster() {
    ...
    const spikeSet = new Set(lastSpikeIndices);
    const rowH = h / rasterSample.length;
    rasterCtx.fillStyle = '#C24B86';
    for (let r = 0; r < rasterSample.length; r++) {
      if (spikeSet.has(rasterSample[r])) rasterCtx.fillRect(w - 1, r * rowH, 1, Math.max(1, rowH));
    }
  }
  ...
  function init(opts) {
    ...
    window.NeuronSim = {
      setCount(n) { setNeuronCount(Math.max(10, Math.min(8000, Math.round(n)))); },
      setNoise(v) { noise = v; },
